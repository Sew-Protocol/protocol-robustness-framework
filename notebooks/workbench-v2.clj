...
(clerk/html
 [:div.workbench-container
  ...
  (let [summary (common/read-json "results/test-artifacts/test-summary.json")
        coverage (common/read-json "results/test-artifacts/coverage.json")
        scenarios (sort-by :id (:scenarios coverage))]
    [:<>
     [:div.hero-strip
      [:div.metric-panel [:div.label "Validation Run"] [:div.value (or (:run_id summary) "—")]]
      ...
      [:div.metric-panel [:div.label "Evidence Hash"] [:div.value {:style {:fontSize "1.2rem"}} "8f2a...1b9c"]]]
     ...
     [:div.grid-layout
      ...
      [:div.card {:style {:grid-column "span 12"}}
       ...
       [:div {:style {:display "grid" :gridTemplateColumns "repeat(20, 1fr)" :gap "6px"}}
         (for [s (take 60 scenarios)]
           (let [adv? (some #(str/includes? (str/lower-case (name %)) "adversarial") (or (:threat-tags s) []))]
              [:div {:class "scenario-dot" 
                     :style {:aspectRatio "1" :background (if adv? "#FF9800" "#7ADDDC") :borderRadius "1px" :boxShadow (str "0 0 5px " (if adv? "#FF9800" "#7ADDDC") "44") :position "relative"}}
               [:div.dot-tooltip {:style {:position "absolute" :bottom "120%" :left "50%" :transform "translateX(-50%)" :background "#fff" :color "#000" :padding "4px 8px" :fontSize "8px" :whiteSpace "nowrap" :display "none" :zIndex "100" :fontFamily "JetBrains Mono"}} 
                (or (:title s) "Untitled")]]))]]
       [:style "
      ...
      ;; 3. Honesty Surface
      [:div {:style {:marginTop "40px" :padding "30px" :border "1px dashed #FF9800"}}
