...
{:authorization/id           string       ; unique identifier "fa-<n>"
 ...
 :allowed-action             string       ; action authorized (default "execute-resolution")
...
1. **Grant** — Governance calls `grant-force-authorization` with a workflow-id,
   reason, and timing parameters (`:starts-at`, `:expires-at` XOR `:duration`).
   The action is gated by `with-governance-actor` and creates an authorization
   record in `world[:force-authorisations]`. Emits `:force-authorisation-granted`
   evidence.
...
3. **Execute** — Any resolved actor (not governance-gated) calls
   ...
   On success, computes a scope-hash from the execution context
   (`:authorization/id`, `:authorization/type`, `:held/direction`, `:token`,
   `:amount`, `:held/account`, `:owner/address`, `:held/reason`,
   `:held/workflow-id`) and delegates to `apply-resolution-transition` with
   `:resolution-source :force-authorized` and the full authorization provenance.

   The accounting layer's `adjust-held` verifies the scope-hash matches the
   actual adjustment, marks the authorization as consumed (dual guard: record
   flag `:consumed?` and accounting-level `:force-authorisations/consumed` map),
   and emits `:force-authorisation-executed` evidence capturing the consumed
   post-state.
...
This ensures the authorization proves not just *that* governance approved an
override, but *exactly which custody movement* was authorized — which token,
how much, to whom, for which escrow.
...
| **Provenance schema** | `governance-authorization.v1` | `execution-provenance.v1` | `force-authorisation.v1` |
